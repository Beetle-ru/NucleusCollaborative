...
namespace HeatControl
{
    public partial class ScrapTable : Form
    {
        ...
        public ScrapTable(MixCalculator _owner)
        {
            InitializeComponent();
            ownerFace = _owner;
        ...
        {
            // TODO: This line of code loads data into the 'chemistryDataSet.Scrap' table. You can move, or remove it, as needed.
            this.scrapTableAdapter.Fill(this.chemistryDataSet.Scrap);
            // TODO: This line of code loads data into the 'scrapDataSet.Scrap' table. You can move, or remove it, as needed.
            //ChemistryDataSetTableAdapters.ScrapTableAdapter.Fill(this.scrapDataSet.Scrap);
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
...
#else
            ScrapMixerTableAdapter ada = new ScrapMixerTableAdapter();
            ...
            MixCalculator.WordPool<double> wpTotal = new MixCalculator.WordPool<double>(0.0);
            for (int i = 0; i < gridScrap.RowCount; i++)
            {
            ...
            for (int i = ownerFace.ch_Scrap.m_propsStart; i < ownerFace.ch_Scrap.gridChem.RowCount; i++)
            {
                wpProps.SetWord((string)ownerFace.ch_Scrap.gridChem.Rows[i].Cells[0].Value, (double)ownerFace.ch_Scrap.gridChem.Rows[i].Cells[1].Value);
            }
            ownerFace.ch_Scrap.m_inFP.Clear();
            ownerFace.ch_Scrap.gridChem.Rows.Clear();
            ownerFace.ch_Scrap.gridChem.RowCount = wpTotal.Count;
            for (var rowCnt = 0;
                rowCnt < ownerFace.ch_Scrap.gridChem.RowCount;
                rowCnt++)
            {
                ownerFace.ch_Scrap.gridChem.Rows[rowCnt].Cells[0].Value
                    = wpTotal.ElementAt(rowCnt).Key;
                ownerFace.ch_Scrap.gridChem.Rows[rowCnt].Cells[1].Value
                    = wpTotal.ElementAt(rowCnt).Value;
                ownerFace.ch_Scrap.m_inFP.SetWord(wpTotal.ElementAt(rowCnt).Key,
                    wpTotal.ElementAt(rowCnt).Value);
            }
            ownerFace.ch_Scrap.m_propsStart =
                ownerFace.ch_Scrap.gridChem.RowCount;
            if (wpProps.Count > 0)
            {
                ownerFace.ch_Scrap.gridChem.Rows.Add(wpProps.Count);
                for (var rowCnt = ownerFace.ch_Scrap.m_propsStart; rowCnt < ownerFace.ch_Scrap.gridChem.RowCount; rowCnt++)
                {
                    ownerFace.ch_Scrap.gridChem.Rows[rowCnt].Cells[0].Value
                        = wpProps.ElementAt(rowCnt - ownerFace.ch_Scrap.m_propsStart).Key;
                    ownerFace.ch_Scrap.gridChem.Rows[rowCnt].Cells[1].Value
                        = wpProps.ElementAt(rowCnt - ownerFace.ch_Scrap.m_propsStart).Value;
                    ownerFace.ch_Scrap.m_inFP.SetWord(wpProps.ElementAt(rowCnt - ownerFace.ch_Scrap.m_propsStart).Key,
                        wpProps.ElementAt(rowCnt - ownerFace.ch_Scrap.m_propsStart).Value);

                }
            }
            ownerFace.ch_Scrap.m_readOnLoad = false;
            Close();
        }
        ...
        private void btnClear_Click(object sender, EventArgs e)
        {
            ...
            ownerFace.ch_Scrap.m_readOnLoad = true;
        }
