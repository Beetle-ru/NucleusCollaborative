...
using HeatControl.ChemistryDataSetTableAdapters;
using Implements;
using Oracle.DataAccess.Client;

namespace HeatControl {
    public partial class ChemTable : Form {
        public FlexHelper fex = new FlexHelper("Model.Shixta-I.Chemistry");
#if (!DB_IS_ORACLE)
        ...
        public MixCalculator.WordPool<double> m_inFP = new MixCalculator.WordPool<double>(0.0);
        ...
        private void ChemTable_FormClosing(object sender, FormClosingEventArgs e) {
            ...
            if (m_dataChanged) {
                if (MessageBox.Show("Сохранить изменения?",
                                    "Химия изменилась", MessageBoxButtons.YesNo, MessageBoxIcon.Question) ==
                    DialogResult.Yes) {
                    m_dataChanged = false;
                    if (btnSave.Enabled) {
                        SaveCSVData();
        ...
        private void ChemTable_DoubleClick(object sender, EventArgs e) {
            var f = new KeyGen();
            f.textBox1.Text = makeKey().ToString();
            f.ShowDialog();
        }
